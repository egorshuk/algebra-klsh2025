\documentclass[10pt, twoside]{article}
\usepackage{be-my-concrete, be-my-geometry}
\setcounter{MaxMatrixCols}{15}

\begin{document}

\pagestyle{empty}
\input{misc/abstract.tex}
\newpage

\tableofcontents
\newpage

\setcounter{page}{1}
\pagestyle{fancy}

Алгебра -- наука о структурах, которые описываются с помощью операций и законов. 
Возможно, то что мы будем называть ``алгебра'' -- это не совсем то, что вы привыкли называть ``алгебра''. 
Потому что в школьном курсе алгебры, особенно в старших классах, почему-то изучается анализ, а не сама алгебра.

Первая структура, с которой мы с вами познакомимся -- это группы.
Это одно из самых ``базовых понятий'', но оно же и является центральным. 

\input{sections/permutations}
\input{problems/permutations}
\input{sections/intro-groups}
\input{problems/intro-groups}
\input{sections/lagrange}
\input{problems/lagrange}
\input{sections/homomorphism}
\input{problems/homomorphism}
\input{sections/action}
\input{problems/action}
\input{sections/cong}
\input{problems/cong}
\input{sections/burnside}
\input{problems/burnside}
\input{sections/intro-rings}
\input{problems/intro-rings}
\input{sections/polynoms}
\input{problems/polynoms}
\input{sections/fields}
\input{problems/fields}

\end{document}


